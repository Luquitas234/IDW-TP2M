...
const EliminarSerAloja = () => {
  const [id, setId] = useState('');

  const deleteSubmit = async (event) => {
    event.preventDefault();
    try {
      const response = await axios.delete(`http://localhost:3001/alojamientosServicios/deleteAlojamientoServicio/${id}`);
...
export default EliminarSerAloja;